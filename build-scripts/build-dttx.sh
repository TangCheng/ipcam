...
pushd ${SOURCE_HOME}/live >/dev/null
  ...
  ## make clean
    ...
    then
      # configure
      cp -f config.linux-with-shared-libraries config.armlinux-${CHIP}
      chmod u+w config.armlinux-${CHIP}
      echo "" >> config.armlinux-${CHIP}
      echo "# Options generated by build script" >> config.armlinux-${CHIP}
      echo "COMPILE_OPTS = \$(INCLUDES) -I. -DSOCKLEN_T=socklen_t -D_LARGEFILE_SOURCE=1 -D_FILE_OFFSET_BITS=64 -DLOCALE_NOT_USED -DALLOW_RTSP_SERVER_PORT_REUSE=1 -fPIC" >> config.armlinux-${CHIP}
      echo "C_COMPILER = ${CROSS_COMPILE}gcc" >> config.armlinux-${CHIP}
      echo "CPLUSPLUS_COMPILER = ${CROSS_COMPILE}g++" >> config.armlinux-${CHIP}
      echo "LINK = ${CROSS_COMPILE}g++ -o" >> config.armlinux-${CHIP}
      echo "LINK_OPTS = -L. \$(LDFLAGS)" >> config.armlinux-${CHIP}
      echo "LIBRARY_LINK = ${CROSS_COMPILE}g++ -o " >> config.armlinux-${CHIP}
      echo "" >> config.armlinux-${CHIP}
      ./genMakefiles armlinux-${CHIP} >>${BUILD_LOG} 2>&1 \
